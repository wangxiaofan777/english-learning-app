...
- ✅ Onboarding 三问 + 10 题测评定级（CEFR 映射）
- ✅ 日常/职场双轨道 12 个精编场景 + AI 生成 pipeline
- ✅ AI 对话：SSE 流式回复、更地道说法、语法纠错、生词提示
- ✅ **听力精听**：逐句盲听 → 显示字幕 → 逐句推进，自动连播（TTS），练完计入学习时长
- ✅ **跟读评分**：示范播放 → 端上语音识别 → 词级 LCS 比对打分，逐词高亮；无识别环境自动降级自评
- ✅ 生词本 + FSRS-5 复习调度（内置算法，含单元测试）
- ✅ 每日三件事计划、streak 打卡、学习统计周报
- ✅ H5 / 微信小程序构建通过；App 端随 uni-app 输出
- ⏳ V1：专业发音评测（音素级）、订阅支付、App 流式
