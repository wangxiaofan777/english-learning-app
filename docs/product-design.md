...
| 模块 | 状态 | 说明 |
| --- | --- | --- |
| 登记建档 | ✅ | 游客/微信登录（dev 兜底）、年龄段（少儿/青少年/成人/银发）+ 目标 + 时长、10 题测评 CEFR 定级 |
| 课程目录 | ✅ | 对标考试等级：6 学段方向（小学KET/初中中考/高中高考/大学四六级/职场BEC/出境生活）× 3 档周期（3/6/12 个月，36/72/96 课时），24 话题/方向大纲驱动 |
| 课程体系 | ✅ | 登记后自动派课（方向+周期），课时「对话→精听→跟读」循环、12 个月班含单元复习课，线性解锁完课推进，可切换方向与周期 |
...
| 听力精听 | ✅ | 逐句盲听/字幕/推进（复用场景内容 + TTS），练完计时入统计并推进课时；素材接入 V1.5 |
| 跟读评分 | ✅ | 示范播放 + 端上语音识别 + 词级 LCS 打分逐词高亮，无识别环境降级自评；音素级评测引擎仍为 V1 |
| 计划与激励 | ✅ | 每日三件事（跟随当前课时）、streak 打卡、XP 经验与等级头衔、11 枚成就徽章、每日一句 |
| 轻量练习 | ✅ | 词汇速测（中译英 4 选 1）+ 拼写挑战（听音拼词）双玩法、自由聊天、精听理解小测 |
| 游戏化关卡 | ✅ | 每单元末 Boss 挑战赛：60 秒限时混合关卡（词汇/听音/句子），通关解锁下一单元并计入课时 |
| 习惯工具 | ✅ | 打卡月历（按月视图+连续天数）、成绩分享海报（Canvas 生成）、每日一句 |
| 内容流水线 | ✅ | admin 单篇/批量 LLM 重写模板场景 + 浏览器版内容运营台；重写保持场景 id 不变、课时不断链 |
| 前端 | ✅ | 十七个页面（5 Tab）；H5 与微信小程序构建通过；App 端随 uni-app 输出（非流式降级） |
| 端到端验证 | ✅ | 17/17 API 冒烟 + 浏览器全流程走查 |

与设计的差异（诚实记录）：

- LLM 接入用自研 OpenAI 兼容客户端替代 Spring AI（减少依赖版本风险，接口已抽象可替换）
- 课程目录已对标考试等级与周期（6 方向 × 3/6/12 个月）；非手写话题由模板场景引擎生成（建课零 LLM 成本），配置 LLM 后可通过 admin 接口逐话题重写为真实内容
- 少儿方向当前复用通用内容形态；真正的少儿课程需单独内容制作与未成年人合规评估（§10.5）
- 听力精听与跟读评分提前到本期实现（复用场景内容，端上 TTS/ASR + LCS 打分）；音素级发音评测引擎（腾讯智聆/讯飞）仍按计划在 V1 接入
- 发音评测、精听素材库、订阅支付按 §6 计划延后至 V1/V1.5
